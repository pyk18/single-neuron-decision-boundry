...
class MainWindow(tk.Tk):
	...
	def __init__(self, debug_print_flag=False):
		...
		self.left_frame.grid(row=1, columnspan=2, sticky=tk.N + tk.E + tk.S + tk.W)
		#self.right_frame.grid(row=2, column=1, sticky=tk.N + tk.E + tk.S + tk.W)
		#self.status_bar.grid(row=2, columnspan=2, sticky=tk.N + tk.E + tk.S + tk.W)
...
class LeftFrame:
	...
	def __init__(self, root, master, debug_print_flag=False):
		...
		self.ymax = 11
		self.bias = 0.0
		self.activation_type = "Sym HardLim"
		self.weight_1 = 1
		self.weight_2 = 1
		self.input_values = np.linspace(-10, 10, 100, endpoint=True)
		#self.plot_line = 0
		self.prev_random = 0
		self.prev_random2 = 0
		self.var=np.random.randint(-10,11,size=(4,2))
		self.xx, self.yy = np.meshgrid(self.input_values, self.input_values)
		#########################################################################
		...
		#self.controls_frame.getpack(side="top", fill='both', expand=True)
		#########################################################################
		...
		self.activation_function_dropdown = tk.OptionMenu(self.controls_frame, self.activation_function_variable,
		                                                  "Sym HardLim", "Linear", "Hyperbolic Tangent",command=lambda
				event: self.activation_function_dropdown_callback())
		self.activation_function_variable.set("Sym HardLim")
		self.activation_function_dropdown.pack(side=tk.RIGHT)
		self.weight_1_slider = tk.Scale(self.controls_frame, variable=tk.DoubleVar(), orient=tk.HORIZONTAL,
		                                    from_=-10.0, to_=10.0, resolution=0.01, bg="#DDDDDD",
		                                    activebackground="#FF0000", highlightcolor="#00FFFF",label="Weight 1",
		                                    command=lambda event: self.weight_1_slider_callback())
		self.weight_1_slider.set(self.weight_1)
		self.weight_1_slider.bind("<ButtonRelease-1>", lambda event: self.weight_1_slider_callback())
		#self.input_weight_slider.grid(row=0, column=0, sticky=tk.N + tk.E + tk.S + tk.W)
		self.weight_1_slider.pack(side=tk.LEFT,fill=tk.X)

		self.weight_2_slider = tk.Scale(self.controls_frame, variable=tk.DoubleVar(), orient=tk.HORIZONTAL, from_=-10.0,
		                            to_=10.0, resolution=0.01, bg="#DDDDDD", activebackground="#FF0000",
		                            highlightcolor="#00FFFF",label="Weight 2",
		                            command=lambda event: self.weight_2_slider_callback())
		self.weight_2_slider.set(self.weight_2)
		self.weight_2_slider.bind("<ButtonRelease-1>", lambda event: self.weight_2_slider_callback())
		self.weight_2_slider.pack(side=tk.LEFT, fill=tk.X)
		...
		self.bias_slider.pack(fill=tk.X,side=tk.LEFT)


		#########################################################################
		#  Set up the frame for drop down selection
		#########################################################################
		self.train_button = tk.Button(self.controls_frame, text="Train", fg="blue",command= self.do_training)
		self.train_button.pack(side=tk.RIGHT)
		self.random_var_gen_button = tk.Button(self.controls_frame, text="Create Random Data", fg="blue",command=self.plot_random_num)
		self.random_var_gen_button.pack(side=tk.RIGHT)

	def key_pressed_callback(self, event):
		self.root.status_bar.set('%s', 'Key pressed')

	def plot_random_num(self):
		self.var = np.random.randint(-10,11,size=(4,2))
		if self.prev_random and self.prev_random2:
			self.prev_random.remove()
			self.prev_random2.remove()

		self.axes.set_xlabel('Input')
		self.axes.set_ylabel('Output')

		self.prev_random, = self.axes.plot([item[0] for item in self.var[:2]],[item[1] for item in self.var[:2]],'y^')
		self.prev_random2, = self.axes.plot([item[0] for item in self.var[2:]],[item[1] for item in self.var[2:]],'bs')
	...
	def display_activation_function(self):
		zz = (self.weight_1)*self.xx+(self.weight_2)*self.yy+self.bias
		zz[zz < 0] = -1
		zz[zz > 0] = +1
		self.axes.relim()
		...
		self.axes.set_xlabel('Input')
		self.axes.set_ylabel('Output')
		self.axes.pcolormesh(self.input_values,self.input_values,zz,cmap='RdYlGn')
	...
	def do_training(self):
		...
		err = 0
		for i in range(100):
			if self.activation_type == "Sym HardLim":
				a = 1 if (np.dot(self.var[i%4],[self.weight_1,self.weight_2]) + self.bias) > 0 else -1
				err = self.labels[i%4] - a

			elif self.activation_type == "Hyperbolic Tangent":
				net = np.dot(self.var[i % 4], [self.weight_1, self.weight_2]) + self.bias
				a = np.tanh(net)
				err = self.labels[i%4] - a
			...
			if err:
				self.weight_1 += .05 * err * self.var[i%4][0]
				self.weight_2 += .05 * err * self.var[i%4][1]
				self.bias += .05 * err
				self.display_activation_function()


	def weight_1_slider_callback(self):
		self.weight_1 = np.float(self.weight_1_slider.get())
		self.display_activation_function()

	def weight_2_slider_callback(self):
		self.weight_1 = np.float(self.weight_2_slider.get())
		self.display_activation_function()
